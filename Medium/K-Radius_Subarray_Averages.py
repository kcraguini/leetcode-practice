...
class Solution:
    def getAverages(self, nums: List[int], k: int) -> List[int]:
        ...
        window_size = 2 * k + 1
        n = len(nums)
        avgs = [-1] * n #autofill with -1

        if window_size > n:
            return avgs
        
        prefix = [0] #sets the first value of prefix sum to 0
        for i in range(n):
            prefix.append(nums[i] + prefix[-1])


        # only iterates through k to n-k and leaves the other -1
        for i in range(k, n - k):
            lBound, rBound = i - k, i + k
            subArrSum = prefix[rBound + 1] - prefix[lBound]
            avg = subArrSum // window_size
            avgs[i] = avg
